...
public class MediaFingerprintService {
    ...
    private Fingerprints generate(String perceptual, CheckedGenerator segment,
        ...
        return new Fingerprints(perceptual, optional("video segments", segment), optional("audio", audio));
    }

    private String optional(String kind, CheckedGenerator generator) {
        try {
            return generator.generate();
        } catch (IOException e) {
            log.warn("Could not generate {} fingerprint", kind, e);
            return null;
        }
    }
